...
private:
  ...
  MYSQL_FIELD *field;
  TableMetaData *table_data;
  IndexMetaData *index_data;
    EmbeddedMYSQL ():results (NULL), field (NULL)
  {
    table_data = new TableMetaData[10];	//currently hard coded
    index_data = new IndexMetaData[10];
  }
...
public:
   ...
   */
  int executeMYSQL (std::string query_str, ExplainMetaData * explain_data);
